...
internal sealed class ShopModule : ModuleBase
{
    ...
    {
        "Guess a number" => ToStateWith(ctx, OnGuessANumber, (Number: Random.Shared.Next(1000), Attempts: 10), null),
        "Register" => ToState(ctx, OnUserRegisteredAsync),
        _ => InvalidInput(ctx),
    };

    [Prompt<int>("I thought of a number from 0 to 1000. Can you guess it in 10 tries?")]
    public StateResult OnGuessANumber(PromptStateContext<int> ctx)
    {
        int number, attempts;
        if (!ctx.TryGetData(out (int, int) value))
            return Fail(ctx);
        (number, attempts) = value;
    ...
    [Menu("Game over", ParentStateName = nameof(OnGuessANumber))] // Parent states are needed for back navigation
    ...
    protected override Task<bool> ValidateModelAsync(ModelBindContext ctx, ModelBindingBuilder bindingBuilder, ICollection<ValidationResult> errors)
    {
        if (bindingBuilder.InputProperty.Property.Name is nameof(UserData.Password))
        {
            ...
            {
                errors.Add(new("Your password should contain at least 1 digit, uppercase, lowercase and a special character."));
    ...
    private static bool IsCorrectPassword(string password) =>
        ...
        password.Any("!@#$%^&*()_+[]{}|;:,.<>?".Contains); // Check for at least one special character

    internal readonly record struct UserData(
