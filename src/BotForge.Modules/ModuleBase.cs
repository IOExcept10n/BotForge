...
public abstract class ModuleBase : IDisposable
{
    ...
    /// <summary>
    /// Asynchronously handles the root state of the module.
    /// </summary>
    ...
    protected static StateResult Retry(ModuleStateContext ctx, string? stateData, string? message)
        => new(ctx.CurrentState.Id, stateData ?? ctx.CurrentState.StateData, new(message, null));
    ...
    protected static StateResult RetryWithMessage(ModuleStateContext ctx, string? message)
        => Retry(ctx, ctx.CurrentState.StateData, message);
    ...
    protected static StateResult Retry(ModuleStateContext ctx)
        => Retry(ctx, ctx.CurrentState.StateData, null);
    ...
    protected StateResult Fail(ModuleStateContext ctx, string? stateData, string? message)
    ...
    /// <param name="message">An optional message for the user.</param>
    /// <returns>A <see cref="StateResult"/> indicating the failure action.</returns>
    protected internal StateResult FailWithMessage(ModuleStateContext ctx, string? message)
    ...
    /// <param name="message">An optional message for the user.</param>
    /// <returns>A <see cref="StateResult"/> indicating the failure action.</returns>
    protected StateResult FailWith<T>(ModuleStateContext ctx, T data, string? message = null)
    ...
    /// <param name="ctx">The module state context.</param>
    /// <returns>A <see cref="StateResult"/> indicating the failure action.</returns>
    protected StateResult Fail(ModuleStateContext ctx)
    ...
    protected internal virtual StateResult InvalidInput(ModuleStateContext ctx)
        => Retry(ctx, ctx.CurrentState.StateData, Localize(ctx, InvalidInputKey));
